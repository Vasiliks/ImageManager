...
MESSAGE(){ if [ $LANG = "ru" ]; then echo "$2"; else echo "$1"; fi }
  ...
  if grep -qs 'spark' /proc/stb/info/model ; then
    ...
    MESSAGE "Box not found !!!" "Платформа SPARK не найдена!!!"
    exit
  fi
  rm -rf /tmp/root
  mkdir -p /tmp/root
...
#backup and clear EMUsettings
  ...
  if [[ `echo $CLEAR_EMU | grep W` ]]; then
    ...
    MESSAGE "Wicardd settings deleted" "Настройки Wicardd удалены"
  ...
  if [[ `echo $CLEAR_EMU | grep M` ]]; then
    ...
    MESSAGE "MgCamd settings deleted" "Настройки MgCamd удалены"
  ...
  if [[ `echo $CLEAR_EMU | grep O` ]]; then
   ...
   MESSAGE "Oscam settings deleted"  "Настройки Oscam удалены"
